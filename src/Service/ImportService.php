...
class ImportService
{
    ...
    public function processJob(int $jobId): void
    {
        ...
        $delimiter = ';';
        $handleFirstLine = fopen($filePath, 'r');
        if ($handleFirstLine !== false) {
            $firstLine = fgets($handleFirstLine);
            fclose($handleFirstLine);
            if ($firstLine !== false) {
        ...
        if ($job->getTotalRows() === null) {
            $lineCount = 0;
            $handleCounter = fopen($filePath, 'r');
            if ($handleCounter !== false) {
                while (($row = fgetcsv($handleCounter, 0, $delimiter)) !== false) {
                    // Pomiń puste wiersze
                    if (array_filter($row) !== []) {
                        $lineCount++;
                    }
                }
                fclose($handleCounter);
            }
        ...
        try {
            while (($row = fgetcsv($handle, 0, $delimiter)) !== false) {
                ...
                if (count($batch) >= self::BATCH_SIZE) {
                    if ($job->getImportType() === 'cars') {
                        $stats = $this->importCars($batch, $mapping, $job->getArticleIdentifierField(), $job);
                    } elseif ($job->getImportType() === 'categories') {
                        $stats = $this->importCategories($batch, $mapping, $job);
                    } else {
                        $stats = $this->processBatch($batch, $mapping, $job);
                    }
                    
                    if (!empty($stats['errors'])) {
                        $existingError = $job->getErrorMessage() ? $job->getErrorMessage() . "\n" : '';
                        $newErrors = implode("\n", $stats['errors']);
                        $job->setErrorMessage($existingError . $newErrors);
                    }
                    ...
                    if ($this->entityManager->isOpen()) {
                        $this->entityManager->flush();
                        // Clear the entity manager to save memory and immediately refetch the job
            ...
            if (count($batch) > 0) {
                ...
                // Publish progress specifically prior to ending, ensuring Mercure is refreshed before saving COMPLETED status
    ...
    private function publishProgress(ImportJob $job, ?string $error = null): void
    {
        try {
            ...
            if ($error !== null) {
                $payload['error'] = $error;
            } elseif ($job->getErrorMessage() !== null) {
                $payload['error'] = $job->getErrorMessage();
            }

            $topic = rtrim($this->mercureTopicBaseUrl, '/') . '/import/progress/' . $job->getId();
            $update = new Update($topic, json_encode($payload));
            
            $this->hub->publish($update);
        } catch (\Exception $e) {
            // Mercure publish failure should not crash the main process, 
            // but in dev it's good to have it in logs if something goes wrong.
        }
    }
        ...
        foreach ($mappedData as $index => $row) {
            ...
            try {
                ...
                if (!$article) {
                    ...
                    $stats['updated']++;
                    $stats['errors'][] = "Row $index: Produkt o kodzie $articleCode został zaktualizowany (już istniał).";
                }
                    ...
                    if (isset($row[$csvCol])) {
                        ...
                        if ($entityField === 'id_category') {
                            ...
                            if (is_numeric($catStr)) {
                                ...
                                $catNameLower = mb_strtolower(trim($catStr));
                                if (isset($localCategoriesMap[$catNameLower])) {
                                    $value = $localCategoriesMap[$catNameLower];
                                } else {
                                    // Create category
                                    ...
                                    $categoryLanguage->setIdLanguage($defaultLang->getId());
                                    $categoryLanguage->setName(trim($catStr));
                                    $categoryLanguage->setDescription('');
                                    $this->entityManager->persist($categoryLanguage);
                                    $this->entityManager->flush();

                                    $localCategoriesMap[$catNameLower] = $category->getId();
        ...
        foreach ($mappedData as $index => $row) {
            ...
            try {
                ...
                if (!$existingCar) {
                    ...
                    $stats['existing_cars']++;
                    $stats['errors'][] = "Row $index: Pojazd (hash $hash) już istnieje i został pominięty.";
                }
    ...
    public function importCategories(array $mappedData, array $columnMapping, ?ImportJob $job = null): array
    {
        $stats = [
            'processed' => 0,
            'created' => 0,
        ...
        if ($job) {
            $user = $job->getUser();
            if ($user) {
                $existingActionSetting = $settingRepository->getSetting('import_category_existing_action', $user);
            }
        }
        if (!$existingActionSetting) {
            $existingActionSetting = $settingRepository->getGlobalSetting('import_category_existing_action') ?? 'skip';
        }
        ...
        foreach ($mappedData as $index => $row) {
            ...
            try {
                $nameColumn = array_search('name', $columnMapping);
                if ($nameColumn === false || !isset($row[$nameColumn]) || trim((string)$row[$nameColumn]) === '') {
                    throw new \Exception("Missing Category Name");
                }

                $categoryName = trim((string)$row[$nameColumn]);
                $catNameLower = mb_strtolower($categoryName);
                
                // Read description if exists
                $descriptionColumn = array_search('description', $columnMapping);
                $description = '';
                if ($descriptionColumn !== false && isset($row[$descriptionColumn])) {
                    $description = trim((string)$row[$descriptionColumn]);
                }
                
                // Read parent if exists
                $parentColumn = array_search('id_parent', $columnMapping);
                $parentId = null;
                if ($parentColumn !== false && isset($row[$parentColumn]) && trim((string)$row[$parentColumn]) !== '') {
                    $pstr = trim((string)$row[$parentColumn]);
                    if (is_numeric($pstr)) {
                        $parentId = (int)$pstr;
                    } else {
                        $pstrLower = mb_strtolower($pstr);
                        if (isset($localCategoriesMap[$pstrLower])) {
                            $parentId = $localCategoriesMap[$pstrLower];
                        }
                    }
                }
                ...
                if (isset($localCategoriesMap[$catNameLower])) {
                    if ($existingActionSetting === 'update') {
                        $catId = $localCategoriesMap[$catNameLower];
                        $cat = $this->categoryRepository->find($catId);
                        $catLang = $this->categoryLanguageRepository->findOneBy(['id_category' => $catId, 'id_language' => $defaultLang->getId()]);
                        ...
                        if ($catLang) {
                            if ($description !== '') {
                                $catLang->setDescription($description);
                            }
                    ...
                    $cat = new Category();
                    $cat->setIdParent($parentId ?? 0);
                    $this->entityManager->persist($cat);
                    $this->entityManager->flush();

                    $catLang = new CategoryLanguage();
                    $catLang->setIdCategory($cat->getId());
                    $catLang->setIdLanguage($defaultLang->getId());
                    $catLang->setName($categoryName);
                    $catLang->setDescription($description);
                    $this->entityManager->persist($catLang);
                    $this->entityManager->flush();

                    $localCategoriesMap[$catNameLower] = $cat->getId();
                    $stats['created']++;
                    $this->logAffectedRow($job, 'category', $cat->getId());
                }
