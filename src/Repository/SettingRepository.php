...
class SettingRepository extends ServiceEntityRepository
{
    ...
    /**
     * Reads a global setting (not tied to any user) by its key.
     * Useful for system-wide configuration like import batch sizes.
     */
    public function getGlobalSetting(string $key): ?string
    {
        $setting = $this->getEntityManager()
            ->createQuery(
                'SELECT s FROM App\Entity\Setting s WHERE s.settingKey = :key ORDER BY s.id ASC'
            )
            ->setParameter('key', $key)
            ->setMaxResults(1)
            ->getOneOrNullResult();

        return $setting instanceof Setting ? $setting->getSettingValue() : null;
    }
